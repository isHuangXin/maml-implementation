...
class OmniglotNShot:
        ...
        if not os.path.isfile(os.path.join(root, 'omniglot.npy')):
            ...
            for (img, label) in self.x:
                if label in temp.keys():
                    ...
                    temp[label] = [img]
            self.x = []
            # labels info deserted, each label contains 20 imgs
            ...
            # Free memory
            temp = []
            # sace all dataset into npy file.
    ...
    def load_dat_cache(self, data_pack):
        """
        Collects sevral batch for N-shot learning
        :param data_pack: [cls_num. 20, 84, 84, 1]
        :return: A list with [support_set, support_set_y, target_y] ready to be fed to out networks
        """
    ...
    def next(self, mode='train'):
        ...
        :param model: The name of the splitting (one of "train", "val", "test")
        :return:
        """
        # update cache if indexes is larger cached num
        if self.indexes[mode] >= len(self.datasets_cache[mode]):
            self.indexes[mode] = 0
            self.datasets_cache[mode] = self.load_dat_cache(self.datasets[mode])
...
if __name__ == '__main__':
    ...
    for i in range(1000):
        ...
        viz.image(x_spt[0], nrow=5, win='s_spt', opts=dict(title='x_spt'))
        viz.image(x_qry[0], nrow=15, win='x_qry', opts=dict(title='x_qry'))
